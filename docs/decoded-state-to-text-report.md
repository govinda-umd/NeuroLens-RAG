...
**Caveat surfaced by actually running it**: with the current thin, one-paper corpus, the 3B model over-agreed — it marked a paper about *predicting creative ability* as "SUPPORTS" a decoded right-hand-movement result, apparently because both mention the SomMot network, without engaging with how weak that connection actually is. This is a combination of two known risks: (1) the corpus itself doesn't yet contain genuinely relevant literature (the planned Yeo/Schaefer/Barch/Van Essen additions should help directly, since they're the actual methodological sources behind this pipeline), and (2) small instruction-tuned models are known to be agreeable/sycophantic by default — worth testing whether a stricter prompt (e.g. explicitly requiring a minimum specificity bar before marking SUPPORTS) or a larger model fixes this once the corpus is real.
