...
`src/neurolens/pipeline.py::explain_decoded_window` runs this end-to-end: the Level 0/1 query text → `retrieval.py` (generalized from `01_pdf_ingestion.ipynb` to a multi-paper corpus) → top-k chunks → a structured stance-labeling prompt (`build_llm_prompt`) → a local LLM (`make_mlx_generate_fn`, `mlx-community/Llama-3.2-3B-Instruct-4bit` via `mlx-lm`, Apple-Silicon-native). Confirmed working end-to-end on a real Experiment 4 checkpoint.
