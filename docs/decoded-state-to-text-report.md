...
### Level 2 — retrieval-grounded generation (RAG, properly) — **implemented**

`src/neurolens/pipeline.py::explain_decoded_window` runs this end-to-end: the Level 0/1 query text → `retrieval.py` (generalized from `01_pdf_ingestion.ipynb` to a multi-paper corpus, now with optional cross-encoder reranking via `retrieve_and_rerank`) → top-k chunks → a structured stance-labeling prompt (`build_llm_prompt`) → a local LLM (`make_mlx_generate_fn`, `mlx-community/Llama-3.2-3B-Instruct-4bit` via `mlx-lm`, Apple-Silicon-native). Confirmed working end-to-end on a real Experiment 4 checkpoint, and now measured (not just eyeballed) — see [case1-summary-report.md §8](case1-summary-report.md#8-rag-evaluation-and-cross-encoder-reranking) for retrieval precision/recall and an LLM stance-label spot-check.

**Update after growing the corpus to 6 papers** (Yeo 2011, Schaefer 2018, Barch 2013, Van Essen 2013, plus 2 more the user added): retrieval quality improved substantially — top similarity scores rose from ~0.34 to ~0.46-0.585 — and the LLM's stance-labeling got noticeably more discriminating. On the same right-hand-movement decode, it correctly marked the genuinely relevant Barch 2013 excerpt (which explicitly discusses the "motor mapping task" and "activation of the right hand") as SUPPORTS, while correctly marking general-methodology excerpts from Yeo 2011 and Schaefer 2018 as UNRELATED rather than stretching to connect them. This is a clear improvement over the original one-paper-corpus over-agreement problem (see the resolved caveat below) — suggesting corpus quality, not just model choice, was the dominant factor.

<details>
<summary>Original caveat from the one-paper corpus (resolved by growing the corpus — kept for reference)</summary>

With the original thin, one-paper corpus, the 3B model over-agreed — it marked a paper about *predicting creative ability* as "SUPPORTS" a decoded right-hand-movement result, apparently because both mention the SomMot network, without engaging with how weak that connection actually is. This was a combination of two risks: (1) the corpus itself didn't yet contain genuinely relevant literature, and (2) small instruction-tuned models are known to be agreeable/sycophantic by default. Growing the corpus resolved most of the observed effect; whether a stricter prompt is still needed for genuinely borderline cases remains an open question worth testing.
</details>
