...
## 6. Rollout: one subject first, then the full 174 — both complete

Two genuinely different steps, not one job at two scales: (1) build and smoke-test the single-subject DIPY pipeline (download → CSD → tractography → the Sec 5 arrays) on one subject first, while it's cheap to iterate on bugs; (2) once that's validated, loop over all 174 subjects as a long-running batch job. Deliberately sequenced this way rather than writing the full-batch version first — a bug caught on subject 1 costs minutes; the same bug caught on subject 87 of 174 costs however long those 86 subjects took to (redundantly) process.
...
**Step (2) result, all 174 subjects (2026-08-28 20:16 → 2026-09-01 12:40, ~3.9 days):** **174/174 processed, 0 failures, 0 crashes.** Ran at a steady ~1.95 subjects/hour the entire way — the subprocess-isolation architecture (Sec 3) held for the full multi-day run with none of the silent OS-level kills that repeatedly hit the LiFE investigation. Every subject's three Sec 5 arrays validated present with `sc_streamline_count.npy` nonzero and no degenerate (all-zero) connectomes: mean 50,459 nonzero ROI-pair connections per subject (range 33,740–61,374), mean max streamline count 2,878 per subject (range 1,177–4,731) — consistent with the single-subject smoke test, not an outlier. Total disk footprint for the full cohort: **1.3GB**, against the ~226GB of raw diffusion volumes downloaded and deleted per-subject along the way (Sec 1) — direct evidence the sufficient-statistics design (Sec 5) achieves what it set out to.
