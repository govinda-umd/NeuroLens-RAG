...
**LiFE at full streamline count is memory-infeasible on this hardware, discovered by running it, not estimated in advance.** Fitting LiFE on the full 1.2M-streamline tractogram was silently killed by the OS after 50+ minutes, RSS still climbing, on a 16GB machine — LiFE's published validation (Pestilli et al. 2014) uses far fewer streamlines (order 10^5, not 10^6+) than this project's whole-brain seed density produces. Fix: LiFE fits on a random subsample of 200,000 streamlines (`scripts/dti_sc_pipeline.py`'s `LIFE_MAX_STREAMLINES`) rather than the full tractogram — consistent with how LiFE's own cross-validation already relies on streamline subsets, not a shortcut invented here. Fits in ~2 min at ~2GB RSS. The raw streamline-count and mean-length arrays (Sec 5) still use every streamline; only the LiFE-weighted array is subsampled.
