...
**Compute cost — flagged as unsized, not estimated, until the single-subject test (Sec 6) runs.** Whole-brain probabilistic tractography is CPU/GPU-intensive per subject, unlike the existing fMRI pipeline's fast ROI-timeseries extraction. 174 subjects at an unknown-but-likely-double-digit-minutes-per-subject cost is a genuinely different compute budget than anything run so far in this project.
