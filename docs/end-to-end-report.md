...
- **Case 2 (supervised-contrastive):** brain encoder aligned to 6 frozen-MiniLM condition-description prototypes in a shared 64-d space via one cross-entropy loss (brain→text only — not literal CLIP, since many windows share one prototype and there's no sensible text→brain direction for a closed 6-item vocabulary).
