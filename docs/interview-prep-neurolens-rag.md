...
**Standardization decision, 2026-08-26: the fitted-post-hoc-classifier-head route above is now the standard CAV/TCAV mechanism for Case 2, not the open-vocabulary route (§6.2, §6.6, §8.4.6).** Every case now tests a mapped concept the same way — fit a linear probe/classification head on the frozen backbone's pooled features from labeled examples, then use that head's differentiable logit for CAV/TCAV's directional derivative — Case 1's own trained head, Case 2 and Case 3's post-hoc-fitted head (`fit_post_hoc_classifier`, reused unmodified across both). **This is a going-forward design choice, not a retraction**: `concepts_case2.py::open_vocabulary_concept_direction` is real, working, already-validated code (zero-shot phrase test: TCAV 0.686, P=1.000 across 1000 resamples, §8.4.6) — nothing about it was wrong. It's superseded as the *default* specifically because the claim-mapping step (whether the existing keyword match in §7, or the proposed embedding-similarity soft-mapping in `docs/v2/rag-cav-verification-loop-design.md` §3) always resolves a claim onto a *known* concept before testing it, at which point labeled brain examples exist for that concept and the open-vocabulary route's no-brain-examples-needed property stops being the limiting factor. Standardizing on one mechanism for every case is also what makes a single shared, deterministic synthesis path (§7's still-open Case 1 gap) buildable without three case-specific branches. If a genuinely novel concept outside the known/extended vocabulary ever needs testing with zero labeled examples, the open-vocabulary route is still there and still works — it's just not the first thing this design reaches for.
