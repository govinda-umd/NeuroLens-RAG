...
**The verification loop — the actual novel contribution.** Literature-extracted phrase → mapped onto a testable concept (keyword match) → CAV direction fit/derived → TCAV score compared against the excerpt's stance. **Critical design decision: the AGREE/DISAGREE/UNCLEAR verdict is computed deterministically in code from (stance, TCAV score) — the LLM never decides it.** Unrelated excerpt → no comparison at all (genuine third outcome, not folded into disagreement); supporting + high TCAV → agreement; supporting + low TCAV → flagged disagreement. LLM's only job: narrate an already-determined verdict.
...
4. **Literal zero-shot classification test**: embed a genuinely novel condition never in the 6 training classes (e.g. a hypothetical "elbow movement") and check whether the brain encoder's representation lands closest to that new embedding.
...
**Standardization decision, 2026-08-26: the fitted-post-hoc-classifier-head route above is now the standard CAV/TCAV mechanism for every case, including Case 2.** Every case tests a mapped concept the same way — fit a linear probe/classification head on the frozen backbone's pooled features from labeled examples, then use that head's differentiable logit for CAV/TCAV's directional derivative — Case 1's own trained head, Case 2 and Case 3's post-hoc-fitted head (`fit_post_hoc_classifier`, reused unmodified across both). A claim is always mapped onto a *known* concept before testing it (keyword match today, §7; the proposed embedding-similarity soft-mapping in `docs/v2/rag-cav-verification-loop-design.md` §3 going forward), and labeled brain examples exist for every known concept — so a single, uniform derivation mechanism covers every case with nothing left over. Standardizing on one mechanism is also what makes a single shared, deterministic synthesis path (§7's still-open Case 1 gap) buildable without three case-specific branches.
...
## 12. Proposed design: merging the two RAG tracks into one claim-first verify-then-query loop (2026-08-25, moved to its own doc 2026-08-26)

**Superseded as a standalone section — moved out to avoid two copies drifting apart.** The full spec now lives in [`docs/v2/rag-cav-verification-loop-design.md`](v2/rag-cav-verification-loop-design.md), since it's a real build target rather than interview color, and it's already been through one correction pass this doc's original write-up didn't have (2026-08-26: every case, including Case 2, uses the same standardized fitted-classifier-head CAV derivation — see the standardization note after §11.3; concept-attribution, not prediction-attribution, is the right mechanism for a claim with no associated decode, and there was never really an open design question about which held-out examples to run it on; the second query in question is the refined/second-pass query, not the first; LoRA is probably unnecessary once the verdict step is fully deterministic; MiniLM is an embedding-only encoder, not a generative LLM, and is a separate model from Llama in every fine-tuning discussion). Read the v2 doc directly rather than this paragraph if the design itself comes up — this entry is a pointer, not a summary to quote from.

**Net summary, if asked to describe this design in one breath**: run the corpus-first sweep to get verified, self-consistent claims; for each claim, find which of the 6 trained representations actually depends on it (not just which one's text-derived proxy suggests it might); run attribution on that specific representation to characterize what it relies on; use that as a second, targeted literature query instead of a generic first-pass one; and synthesize the result the same deterministic way the existing loop already had to learn to, the hard way. **Honest gap this doesn't close**: still says nothing new about the Case 1 loop's still-unfixed free-judgment design (§7) — a claim's representation-of-best-fit could easily turn out to be Case 1's, and this design doesn't by itself force that path through the deterministic-verdict rule unless step 6 is built to apply it uniformly regardless of which case originated the claim.
