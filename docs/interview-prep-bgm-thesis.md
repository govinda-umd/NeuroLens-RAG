...
- [ ] Chapter 3: human threat/reward imminence processing (pages 97-143) -- same generative-modeling methodology applied to human fMRI, a separate published paper (Murty, Song, Surampudi & Pessoa, *J. Neuroscience* 2023). Scope question to resolve with user: does this belong under the same resume project entry as the mouse work, or stays a separate supporting publication?
...
- [ ] Supplementary Section for Chapter 2 (pages 149-171): the actual mathematical formulations (SBM likelihood, description length derivation, MCMC acceptance criteria) -- read if the interview format seems likely to probe equation-level detail.
- [ ] Resume points for the BGM project (draft once enough material gathered, same process as NeuroLens-RAG).
