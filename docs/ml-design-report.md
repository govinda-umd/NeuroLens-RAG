...
3. ~~**`lambda_hrf` tuning strategy**~~ — **grid-searched** in [`07_hyperparameter_sweep.ipynb`](../notebooks/07_hyperparameter_sweep.ipynb) over `[0.0, 0.05, 0.1, 0.3, 1.0]`. At `lambda_hrf=0.0` the HRF head is essentially untrained (MSE ~0.34-0.35, since a loss term scaled by 0 contributes no gradient) — confirms the auxiliary loss is necessary, not just beneficial. Classification is fairly robust to lambda_hrf for both encoders; GRU's HRF fit improves with higher lambda at ~zero classification cost, while Transformer has a real (small) tradeoff, peaking at 0.1 for classification and 1.0 for HRF fit. Uncertainty-weighted multi-task loss (Kendall et al.) remains untried, still a candidate if a fixed scalar proves limiting later.
...
6. **Optimizer/LR/batch-size/epoch-count sweep** — epoch count was swept up to 20 in [`07_hyperparameter_sweep.ipynb`](../notebooks/07_hyperparameter_sweep.ipynb): more epochs did **not** reliably improve test macro F1 (3 of 4 experiments non-monotonic or declining past 5-10 epochs), even though the validation-selected checkpoint's own macro F1 kept creeping up — a generalization-gap symptom of only having 3 val / 3 test subjects, not a training-length problem. 5 epochs remains reasonable at this scale. Optimizer/LR/batch-size themselves are still untuned (AdamW, lr=1e-3, batch=64 throughout) — lower priority now than growing the subject count further.
