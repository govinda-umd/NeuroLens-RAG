...
This turns the literature into a source of testable hypotheses about *what the model might be representing*, rather than only a source of text to compare the decode against — the RAG and interpretability halves of the pipeline stop being sequential stages and become a loop. **The literature-derived version is still not implemented** — the open design question (turning an extracted concept *phrase* into the labeled example set TCAV needs, without heavy manual curation) is unchanged. **What's now implemented is the TCAV mechanism itself**, validated on label-derived concepts (`src/neurolens/concepts.py`, [`09_concept_vectors.ipynb`](../notebooks/09_concept_vectors.ipynb)) — concepts built from the true movement labels already on disk (`hand`, `foot`, `tongue`, `right_side`, `left_side`) rather than from retrieved literature. This validates the mechanism against ground truth before ever attempting the harder literature-derived version described above: it's the same `train_cav`/`tcav_score` machinery, just with the "how do we get labeled examples for this concept" question answered by existing labels instead of retrieval.

**Result**: the effector concepts (`hand`, `foot`, `tongue`) show clean, exactly-as-expected TCAV scores (1.0 for the classes defining the concept, 0.0 elsewhere, >0.99 probe accuracy) — real evidence the representation's directions correspond to human-interpretable concepts, not just to raw classification separability. The laterality concepts (`right_side`/`left_side`) surfaced an honest limitation worth remembering for the literature-derived version too: a CAV fit only on a subset of classes (lateralized movements) extrapolated in a directionally consistent but not necessarily meaningful way onto classes outside that subset (`baseline` and `tongue` both scored 1.0 on `right_side`) — a reminder that a CAV score on inputs outside its training distribution isn't automatically trustworthy.
