...
**Revised.** The first version of this section defaulted to a generic "ML engineer" bullet template and lost the actual sophistication of the work — no mention of representation learning, concept-based interpretability, or the vocabulary a PhD-level ML/AI hiring committee would expect. This version corrects that: it describes Case 1 as what it actually is (a multi-task **representation learning** result, validated two independent ways — decoding accuracy and concept activation vectors — not merely a classifier), while keeping the systems/engineering evidence (RAG, local LLM deployment, production debugging) as a deliberate complement to the Bayesian-statistics depth already established elsewhere on the resume. **Only completed work is included** — Case 2 (contrastive multimodal representation learning) and Case 3 (Bayesian dynamical-systems modeling) are real, scoped next steps (see [case2-3-design-plan.md](case2-3-design-plan.md)), not yet real accomplishments, so they don't belong on a resume yet.

**NeuroLens-RAG: Multi-Task Representation Learning and Interpretable, Literature-Grounded Neural Decoding** — 2026

- Learned a multi-task neural representation of human fMRI motor-cortex dynamics — jointly optimized for discriminative decoding and generative hemodynamic-response regression — achieving **92.5% test macro-F1** across 6 movement classes on held-out subjects, and validated the representation's semantic structure with **Concept Activation Vectors (TCAV)**, confirming that effector- and laterality-specific concepts are linearly encoded as distinct, separable directions in the learned embedding space.
- Diagnosed an apparent negative-R² anomaly in the auxiliary generative objective as a data-scarcity artifact rather than a modeling flaw, by designing a controlled ablation across model architecture and a **26-configuration hyperparameter sweep** (loss-weighting, training duration), fully resolving it (R² 0.61–0.73) after scaling training data 4×.
- Quantified a systematic, reproducible disagreement between gradient-based (Saliency, Integrated Gradients) and perturbation-based (exact Shapley values, LIME) attribution methods for localizing decoded brain states to canonical resting-state networks, making **exact (non-sampled) Shapley-value computation tractable** across **762 test samples in under 5 minutes** via a network-level coalition reformulation.
- Built and evaluated a fully local retrieval-augmented generation (RAG) system — dense retrieval with cross-encoder reranking over a curated scientific corpus, paired with a quantized 3B-parameter LLM (Llama-3.2 via Apple's MLX) — that grounds model predictions in literature through a citation-constrained generation prompt; **measured, not assumed**, quality via a hand-labeled evaluation set, improving retrieval recall from **80% to 100%** and identifying a specific factual-accuracy failure mode in the LLM's generated scientific claims.
- Diagnosed and resolved a production-breaking PyTorch dependency conflict introduced by an unrelated ML library's transitive dependency chain, restoring full training-pipeline functionality within minutes via targeted, isolated dependency reinstallation, with zero loss of in-progress experimental results.
...
**4. Validated that a supervised multi-task model actually learned meaningful representations, not just a good classifier.**
*Situation*: The project's real goal was a learned *representation* of brain activity, not just decoding accuracy — but accuracy alone can't distinguish "the representation captures real structure" from "the classifier head memorized a shortcut."
*Task*: Independently test whether the model's internal representation organizes information along human-interpretable concepts, using a method separate from the classification task itself.
*Action*: Implemented Concept Activation Vectors (TCAV) — defined five concepts from movement semantics (which limb, which side of the body), fit linear probes for each in the model's shared 128-dimensional representation, and measured each class's sensitivity to moving along those directions.
*Result*: Effector concepts (hand, foot, tongue) showed clean, near-perfect separation exactly matching their intended classes, real evidence the representation is organized meaningfully, not just linearly separable by coincidence. The laterality concepts also surfaced a genuine limitation — a direction fit only on lateralized movements extrapolated in a specific but not necessarily meaningful way onto unrelated classes — which I documented rather than omitted, since knowing where an interpretability method's claims stop being trustworthy is as important as the method working in the first place.
